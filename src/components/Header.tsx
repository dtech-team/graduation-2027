...
import { Sparkles, MapPin, PlusCircle, Menu, X, Camera, Heart, ShieldCheck, Clock, User } from "lucide-react";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            >
              {vipUser?.status === "approved" ? (
                <>
                  <Image src="/icons/authen3.png" alt="authen" width={20} height={20} />
                  <span className="hidden sm:inline truncate max-w-[110px]">{vipUser.googleName}</span>
                  <span className="text-[10px] bg-black text-white px-1.5 py-0.2 rounded-full">VIP</span>
                </>
              ) : vipUser?.status === "pending" ? (
                <>
                ...
                <>
                  <Image src="/icons/login3.png" alt="login" width={20} height={20} />
                  <span className="text-[11px] uppercase sm:text-xs">Đăng Nhập</span>
                </>
              )}
            </button>
            ...
            <Link
              href="/"
              className="relative group bg-gradient-to-r from-[#ff3af2] to-[#ab00a3] text-white font-display font-black text-xs sm:text-sm px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border-2 border-black shadow-[3px_3px_0px_0px_#00f2d1] hover:scale-105 hover:shadow-[5px_5px_0px_0px_#fde400] active:scale-95 active:shadow-none transition-all uppercase tracking-wider flex items-center gap-2 select-none"
            >
              <Image src="/icons/wish1.png" alt="wish" width={22} height={22} className="shrink-0" />
              <span className="drop-shadow-[1px_1px_0_#000] hidden sm:inline">TẠO THIỆP</span>
            </Link>

            {/* Mobile Menu Toggle Button */}
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="lg:hidden w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#221033] border-2 border-secondary-fixed text-secondary-fixed flex items-center justify-center shadow-[2px_2px_0px_0px_#000] active:translate-y-0.5 cursor-pointer"
          ...
          {mobileMenuOpen && (
            ...
            >
              <button
                ...
                className="flex items-center justify-between p-3 rounded-xl bg-[#251036] border border-secondary-fixed text-white font-display font-black text-xs sm:text-sm uppercase tracking-wider hover:bg-secondary-fixed hover:text-black transition-colors text-left"
              >
                <span className="flex items-center gap-2.5">
                  <ShieldCheck className="w-4 h-4 text-secondary-fixed" />
                  {vipUser ? `Tài Khoản: ${vipUser.googleName} (${vipUser.status})` : "Xác Thực Khách VIP / Google"}
                </span>
